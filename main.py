...
# In-memory Store for Screenings with realistic seed data
SCREENINGS_STORE = {}

# Populate seed verification history for realistic immigration dashboard
SEED_HISTORY = [
    {
        ...
        "image_url": "/static/images/presets/sample_genuine_passport.jpg"
    },
    {
        ...
        "image_url": "/static/images/presets/sample_tampered_dob.jpg"
    },
    {
        ...
        "image_url": "/static/images/presets/sample_photo_tampered.jpg"
    },
    {
        ...
        "image_url": "/static/images/presets/sample_invalid_mrz.jpg"
    }
]
...
async def page_dashboard(request: Request):
    ...
    return templates.TemplateResponse(
        "index.html",
        {"request": request, "page_title": "AuthenSight - Dashboard", "current_page": "dashboard"}
    )

@app.get("/screening", response_class=HTMLResponse)
async def page_screening(request: Request):
    """Render Document Screening & Analysis Page."""
    return templates.TemplateResponse(
        "screening.html",
        {"request": request, "page_title": "AuthenSight - Document Screening", "current_page": "screening"}
    )

@app.get("/history", response_class=HTMLResponse)
async def page_history(request: Request):
    """Render Verification History Page."""
    return templates.TemplateResponse(
        "history.html",
        {"request": request, "page_title": "AuthenSight - Verification History", "current_page": "history"}
    )

@app.get("/reports", response_class=HTMLResponse)
async def page_reports_overview(request: Request):
    """Render Reports Overview Page."""
    latest_id = list(SCREENINGS_STORE.keys())[0] if SCREENINGS_STORE else None
    return templates.TemplateResponse(
        "report.html",
        {"request": request, "page_title": "AuthenSight - Screening Report", "screening_id": latest_id, "current_page": "reports"}
    )

@app.get("/report/{screening_id}", response_class=HTMLResponse)
async def page_report_detail(request: Request, screening_id: str):
    """Render Specific Screening Report Dossier."""
    return templates.TemplateResponse(
        "report.html",
        {"request": request, "page_title": f"Report - {screening_id}", "screening_id": screening_id, "current_page": "reports"}
    )
...
async def api_screen_document(
    ...
    if file and file.filename:
        ...
        # Read file with size check (limit 25MB)
        ...
        image_rel_url = f"/static/images/presets/{preset_file}"
        preset_abs = os.path.join(STATIC_DIR, "images", "presets", preset_file)
...
async def api_verify_face(
    ...
    """Dedicated Face Verification Endpoint."""
    file_bytes = None
    filename = None
    if file:
        filename = file.filename
...
async def api_validate_mrz(req: MRZValidateRequest):
    """Dedicated MRZ Validation Endpoint."""
    mrz_lines = []
...
async def api_get_history(
    ...
    """Get list of past screenings with filter support."""
...
async def api_get_stats():
    """Get dashboard screening statistics."""
