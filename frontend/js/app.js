 ...
 * Works seamlessly whether hosted via FastAPI or served as standalone static frontend.
 */
...
async function loadRecentScreenings() {
    ...
    try {
        ...
        tbody.innerHTML = records.map(rec => {
            ...
            const reportUrl = rec.screening_id ? `report.html?id=${rec.screening_id}` : `report.html`;
            return `
                <tr>
                    ...
                    <td>
                        <a href="${reportUrl}" class="btn btn-secondary btn-xs">View Dossier</a>
                    </td>
                </tr>
            `;
        }).join("");
...
function onPresetSelectChange(presetValue) {
    ...
    const imgPath = `images/presets/${target.file}`;
    if (previewImage) previewImage.src = imgPath;
...
async function handleScreeningSubmit(e) {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE}/api/screen-document`, {
            method: "POST",
            body: formData
        });
...
function renderScreeningResults(data) {
    ...
    if (heatmapImg) {
        ...
        if (data.image_url) {
            // If relative url from backend, prepend API_BASE if standalone
            const fullUrl = data.image_url.startsWith("http") ? data.image_url : `${API_BASE}${data.image_url}`;
            heatmapImg.src = fullUrl;
...
function openPrintableReport() {
    if (currentScreeningResult && currentScreeningResult.screening_id) {
        window.open(`report.html?id=${currentScreeningResult.screening_id}`, "_blank");
    } else {
        window.print();
    }
}
...
function renderHistoryTable(records) {
    ...
    tbody.innerHTML = records.map(rec => {
        ...
        const reportUrl = `report.html?id=${rec.screening_id}`;
        return `
            <tr>
                <td class="font-mono"><strong>${rec.screening_id}</strong></td>
                <td>${rec.document_type || "Passport"}</td>
                ...
                <td><span class="status-badge ${statusClass}">${rec.status}</span></td>
                <td>
                    <a href="${reportUrl}" class="btn btn-secondary btn-xs">Dossier</a>
                </td>
            </tr>
        `;
    }).join("");
...
async function loadReportDossier(screeningId) {
    // If not provided in argument, check query parameter
    if (!screeningId) {
        const urlParams = new URLSearchParams(window.location.search);
        screeningId = urlParams.get("id");
    }

    try {
        let data = null;
        if (screeningId) {
            const res = await fetch(`${API_BASE}/api/screening-result/${screeningId}`);
            if (res.ok) {
                data = await res.json();
            }
        }
        ...
        if (!data) {
            // Fallback to latest item from history
            const histRes = await fetch(`${API_BASE}/api/history?limit=1`);
            if (histRes.ok) {
        ...
        if (data) {
            ...
            console.warn("No screening dossier data found");
        }
    } catch (err) {
